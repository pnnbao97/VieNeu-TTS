...
# Configuration
DICT_DIR = os.getenv("PHONEME_DICT_DIR", os.path.join(os.path.dirname(__file__), "phone_dict"))
...
def _setup_windows_espeak() -> None:
    ...
    if os.path.exists(default_path):
        ...
        logger.warning(
            "\033[91;1m⚠️ eSpeak-NG is not installed. The system will use the built-in dictionary, but it is recommended to install eSpeak-NG for maximum performance and accuracy.\033[0m"
        )
...
def _setup_linux_espeak() -> None:
    ...
    logger.warning(
        "\033[91;1m⚠️ eSpeak-NG is not installed on Linux. The system will use the built-in dictionary, but it is recommended to install eSpeak-NG (sudo apt install espeak-ng) for maximum performance.\033[0m"
    )
...
def _setup_macos_espeak() -> None:
    ...
    logger.warning(
        "\033[91;1m⚠️ eSpeak-NG is not installed on macOS. The system will use the built-in dictionary, but it is recommended to install eSpeak-NG (brew install espeak-ng) for maximum performance.\033[0m"
    )
    ...
    while i < n:
        if tokens[i]["lang"] == "common":
            ...
            for l in range(start - 1, -1, -1):
                if tokens[l]["content"] in _STOP_PUNCT:
                    break
            ...
            for r in range(end + 1, n):
                if tokens[r]["content"] in _STOP_PUNCT:
                    break
...
def phonemize_batch(texts: list[str], skip_normalize: bool = False, phoneme_dict: dict = None, **kwargs) -> list[str]:
    ...
    for sent in batch_token_lists:
        for t in sent:
            if t["lang"] == "punct" or t.get("force_espeak"):
                continue
            lw = t["content"].lower()
    ...
    if global_unknown:
        ...
        def has_accent(w):
            return any(c in _VI_ACCENTS for c in w.lower())
...
if __name__ == "__main__":
    ...
    test_text = (
        " ".join(sys.argv[1:])
        if len(sys.argv) > 1
        else "Đọc kết quả sẽ giúp hiểu rõ hơn sở thích và thói quen của thế hệ Z."
    )
